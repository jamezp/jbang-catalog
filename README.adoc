...
You can pass more than one pull request to the command to allow additional pull requests to be updated with the JIRA.
Only the first pull request is used to create the JIRA. Each additional pull request is simply updated with the JIRA
information and a link to the pull request is added to the JIRA.

TIP: Use this option when you have the same version upgrade on two different branches.
